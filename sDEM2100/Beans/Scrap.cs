...
namespace sDEM2100.Beans
{
    class Scrap
    {
        ...
        public Scrap(object[] clientInfo, string dbName, string plant)
        {
        ...
        {
            double result = (0.2 - actual) / 0.2 * weight * 100;
        ...
        public ExecutionResult DoOpreater(int id)
        {
            ...
            DataTable dtScrap = dqmsSqlOperater.GetDqmsParams(id, plant);
            ...
            {
                string frequency = dtScrap.Rows[0]["frequency"].ToString();
                dqmsBean.KpiId = id;
                dqmsBean.BuCode = dtScrap.Rows[0]["bu_code"].ToString();
                dqmsBean.PlantCode = plant;
                DataTable dtSource = dqmsSqlOperater.GetScrap(plant, "Operation", "CPB");
                ...
                if (dtSource != null && dtSource.Rows.Count > 0)
                {
                    ...
                    double weight = double.Parse(dtScrap.Rows[0]["weight"].ToString());
                    double lowerlimit = double.Parse(dtScrap.Rows[0]["lower_limit"].ToString());
                    iniTrend = int.Parse(dtScrap.Rows[0]["trend"].ToString());
                    dqmsBean.Score = Scrap.ComputationScore(dqmsBean.Actual, weight, lowerlimit);
                    ...
                    if (dtTmp != null && dtTmp.Rows.Count > 0)
                    {
                        ...
                        if (oldWorkDate != dqmsBean.WorkDate)
                        {
                            ...
                            dqmsBean.Trend = Scrap.CheckTrend(dqmsBean.Actual, dqmsBean.LastActual, iniTrend);

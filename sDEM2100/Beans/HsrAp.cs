...
namespace sDEM2100.Beans
{
    class HsrAp
    {
        ...
        public HsrAp(object[] clientInfo, string dbName, string plant)
        {
        ...
        public ExecutionResult DoOpreater(int id)
        {
            ...
            DataTable dtHsrAp = dqmsSqlOperater.GetDqmsParams(id, plant);
            ExecutionResult exeRes = new ExecutionResult();
            exeRes.Status = true;
            exeRes.Message = string.Empty;
            int iniTrend = 0;
            if (dtHsrAp != null && dtHsrAp.Rows.Count > 0)
            {
                string frequency = dtHsrAp.Rows[0]["frequency"].ToString();
                dqmsBean.KpiId = id;
                dqmsBean.BuCode = dtHsrAp.Rows[0]["bu_code"].ToString();
                dqmsBean.PlantCode = plant;
                DataTable dtSource = dqmsSqlOperater.GetHsrApSmt(plant, "AP");
                ...
                if (dtSource != null && dtSource.Rows.Count > 0)
                {
                    ...
                    double weight = double.Parse(dtHsrAp.Rows[0]["weight"].ToString());
                    //double lowerlimit = double.Parse(dtHsrAp.Rows[0]["lower_limit"].ToString());
                    iniTrend = int.Parse(dtHsrAp.Rows[0]["trend"].ToString());
                    dqmsBean.Score = HsrAp.ComputationScore(dqmsBean.Actual, weight);
                    ...
                    if (dtTmp != null && dtTmp.Rows.Count > 0)
                    {
                        ...
                        if (oldWorkDate != dqmsBean.WorkDate)
                        {
                            ...
                            dqmsBean.Trend = HsrAp.CheckTrend(dqmsBean.Actual, dqmsBean.LastActual, iniTrend);

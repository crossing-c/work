...
namespace sDEM2100.Beans
{
    class OpMonth
    {
        ...
        public OpMonth(object[] clientInfo, string dbName, string plant)
        {
        ...
        public ExecutionResult DoOpreater(int id)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                var dtSource = dqmsSqlOperater.GetOpMonth(plant, WorkDateConvert.GetWorkDate("MONTHLY", -1));

                string sWorkDate = WorkDateConvert.GetWorkDate("MONTHLY", -1), sValue = "0";
                double sdValue = 0;
                if (dtSource != null && dtSource.Rows.Count > 0)
                {
                    sValue = dtSource.Rows[0][0].ToString();
                    sWorkDate = string.IsNullOrEmpty(dtSource.Rows[0][1].ToString())? sWorkDate: dtSource.Rows[0][1].ToString();

...
namespace sDEM2100.Beans
{
    class PidMe
    {
        ...
        public PidMe(object[] clientInfo, string dbName, string plant)
        {
        ...
        {
            double result = (15000 - actual) / 15000 * weight * 100;
        ...
        public ExecutionResult DoOpreater(int id)
        {
            ...
            DataTable dtPidMe = dqmsSqlOperater.GetDqmsParams(id, plant);
            ExecutionResult exeRes = new ExecutionResult();
            exeRes.Status = true;
            exeRes.Message = string.Empty;
            int iniTrend = 0;
            if (dtPidMe != null && dtPidMe.Rows.Count > 0)
            {
                string frequency = dtPidMe.Rows[0]["frequency"].ToString();
                dqmsBean.KpiId = id;
                dqmsBean.BuCode = dtPidMe.Rows[0]["bu_code"].ToString();
                dqmsBean.PlantCode = plant;
                DataTable dtSource = dqmsSqlOperater.GetPidMe(plant);
                ...
                if (dtSource != null && dtSource.Rows.Count > 0)
                {
                    ...
                    double weight = double.Parse(dtPidMe.Rows[0]["weight"].ToString());
                    double lowerlimit = double.Parse(dtPidMe.Rows[0]["lower_limit"].ToString());
                    iniTrend = int.Parse(dtPidMe.Rows[0]["trend"].ToString());
                    dqmsBean.Score = PidMe.ComputationScore(dqmsBean.Actual, weight, lowerlimit);
                    ...
                    if (dtTmp != null && dtTmp.Rows.Count > 0)
                    {
                        ...
                        if (oldWorkDate != dqmsBean.WorkDate)
                        {
                            ...
                            dqmsBean.Trend = PidMe.CheckTrend(dqmsBean.Actual, dqmsBean.LastActual, iniTrend);

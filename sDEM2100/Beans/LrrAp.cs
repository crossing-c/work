...
namespace sDEM2100.Beans
{
    class LrrAp
    {
        ...
        public LrrAp(object[] clientInfo, string dbName, string plant)
        {
        ...
        public ExecutionResult DoOpreater(int id)
        {
            ...
            DataTable dtLrrAp = dqmsSqlOperater.GetDqmsParams(id, plant);
            ExecutionResult exeRes = new ExecutionResult();
            exeRes.Status = true;
            exeRes.Message = string.Empty;
            int iniTrend = 0;
            if (dtLrrAp != null && dtLrrAp.Rows.Count > 0)
            {
                string frequency = dtLrrAp.Rows[0]["frequency"].ToString();
                dqmsBean.KpiId = id;
                dqmsBean.BuCode = dtLrrAp.Rows[0]["bu_code"].ToString();
                dqmsBean.PlantCode = plant;
                DataTable dtSource = dqmsSqlOperater.GetLrrApSmt(plant, "AP");
                ...
                if (dtSource != null && dtSource.Rows.Count > 0)
                {
                    ...
                    double weight = double.Parse(dtLrrAp.Rows[0]["weight"].ToString());
                    //double lowerlimit = double.Parse(dtLrrAp.Rows[0]["lower_limit"].ToString());
                    iniTrend = int.Parse(dtLrrAp.Rows[0]["trend"].ToString());
                    dqmsBean.Score = LrrAp.ComputationScore(dqmsBean.Actual, weight);
                    ...
                    if (dtTmp != null && dtTmp.Rows.Count > 0)
                    {
                        ...
                        if (oldWorkDate != dqmsBean.WorkDate)
                        {
                            ...
                            dqmsBean.Trend = LrrAp.CheckTrend(dqmsBean.Actual, dqmsBean.LastActual, iniTrend);

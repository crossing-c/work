...
namespace sDEM2100.Beans
{
    class ReflowRate
    {
        ...
        public ReflowRate(object[] clientInfo, string dbName, string plant)
        {
        ...
        public ExecutionResult DoOpreater(int id,string str1)
        {
            DqmsBean dqmsBean = new DqmsBean();
            DataTable dtReflowRate = dqmsSqlOperater.GetDqmsParams(id, plant);
            ExecutionResult exeRes = new ExecutionResult();
            exeRes.Status = true;
            exeRes.Message = string.Empty;
            int iniTrend = 0;
            if (dtReflowRate != null && dtReflowRate.Rows.Count > 0)
            {
                string frequency = dtReflowRate.Rows[0]["frequency"].ToString();
                dqmsBean.KpiId = id;
                dqmsBean.BuCode = dtReflowRate.Rows[0]["bu_code"].ToString();
                dqmsBean.PlantCode = plant;
                DataTable dtSource = dqmsSqlOperater.GetReflowRate(plant.Replace("TW01","CTY1"), str1);
                ...
                if (dtSource != null && dtSource.Rows.Count > 0)
                {
                    ...
                    double weight = double.Parse(dtReflowRate.Rows[0]["weight"].ToString());
                    double lowerlimit = double.Parse(dtReflowRate.Rows[0]["lower_limit"].ToString());
                    iniTrend = int.Parse(dtReflowRate.Rows[0]["trend"].ToString());
                    dqmsBean.Score = ReflowRate.ComputationScore(dqmsBean.Actual, weight,lowerlimit);
                    ...
                    if (dtTmp != null && dtTmp.Rows.Count > 0)
                    {
                        ...
                        if (oldWorkDate != dqmsBean.WorkDate)
                        {
                            ...
                            dqmsBean.Trend = ReflowRate.CheckTrend(dqmsBean.Actual, dqmsBean.LastActual, iniTrend);

...
namespace sDEM2100.Beans
{
    class PidLcd
    {
        ...
        public PidLcd(object[] clientInfo, string dbName, string plant)
        {
        ...
        {
            double result = (1250 - actual) / 1250 * weight * 100;
        ...
        public ExecutionResult DoOpreater(int id)
        {
            ...
            DataTable dtPidLcd = dqmsSqlOperater.GetDqmsParams(id, plant);
            ExecutionResult exeRes = new ExecutionResult();
            exeRes.Status = true;
            exeRes.Message = string.Empty;
            int iniTrend = 0;
            if (dtPidLcd != null && dtPidLcd.Rows.Count > 0)
            {
                string frequency = dtPidLcd.Rows[0]["frequency"].ToString();
                dqmsBean.KpiId = id;
                dqmsBean.BuCode = dtPidLcd.Rows[0]["bu_code"].ToString();
                dqmsBean.PlantCode = plant;
                DataTable dtSource = dqmsSqlOperater.GetPidLcd(plant);
                ...
                if (dtSource != null && dtSource.Rows.Count > 0)
                {
                    ...
                    double weight = double.Parse(dtPidLcd.Rows[0]["weight"].ToString());
                    double lowerlimit = double.Parse(dtPidLcd.Rows[0]["lower_limit"].ToString());
                    iniTrend = int.Parse(dtPidLcd.Rows[0]["trend"].ToString());
                    dqmsBean.Score = PidLcd.ComputationScore(dqmsBean.Actual, weight, lowerlimit);
                    ...
                    if (dtTmp != null && dtTmp.Rows.Count > 0)
                    {
                        ...
                        if (oldWorkDate != dqmsBean.WorkDate)
                        {
                            ...
                            dqmsBean.Trend = PidLcd.CheckTrend(dqmsBean.Actual, dqmsBean.LastActual, iniTrend);

...
namespace sDEM2100.Beans
{
    class Ctq
    {
        ...
        public Ctq(object[] clientInfo, string dbName, string plant)
        {
        ...
        public ExecutionResult DoOpreater(int id)
        {
            ...
            DataTable dtCtq = dqmsSqlOperater.GetDqmsParams(id, plant);
            ExecutionResult exeRes = new ExecutionResult();
            exeRes.Status = true;
            exeRes.Message = string.Empty;
            int iniTrend = 0;
            if (dtCtq != null && dtCtq.Rows.Count > 0)
            {
                string frequency = dtCtq.Rows[0]["frequency"].ToString();
                dqmsBean.KpiId = id;
                dqmsBean.BuCode = dtCtq.Rows[0]["bu_code"].ToString();
                dqmsBean.PlantCode = plant;
                DataTable dtSource = dqmsSqlOperater.GetCTQ(plant, "AP");
                ...
                if (dtSource != null && dtSource.Rows.Count > 0)
                {
                    ...
                    double weight = double.Parse(dtCtq.Rows[0]["weight"].ToString());
                    double lowerlimit = double.Parse(dtCtq.Rows[0]["lower_limit"].ToString());
                    iniTrend = int.Parse(dtCtq.Rows[0]["trend"].ToString());
                    dqmsBean.Score = Ctq.ComputationScore(dqmsBean.Actual, weight, lowerlimit);
                    ...
                    if (dtTmp != null && dtTmp.Rows.Count > 0)
                    {
                        ...
                        if (oldWorkDate != dqmsBean.WorkDate)
                        {
                            ...
                            dqmsBean.Trend = Ctq.CheckTrend(dqmsBean.Actual, dqmsBean.LastActual, iniTrend);

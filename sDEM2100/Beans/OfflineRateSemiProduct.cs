...
namespace sDEM2100.Beans
{
    class OfflineRateSemiProduct
    {
        ...
        public OfflineRateSemiProduct(object[] clientInfo, string dbName, string plant)
        {
        ...
        {
            double result = (10 - actual) / 10 * weight * 100;
        ...
        public ExecutionResult DoOpreater(int id)
        {
            ...
            DataTable dtOfflineRateSemiProduct = dqmsSqlOperater.GetDqmsParams(id, plant);
            ExecutionResult exeRes = new ExecutionResult();
            exeRes.Status = true;
            exeRes.Message = string.Empty;
            int iniTrend = 0;
            if (dtOfflineRateSemiProduct != null && dtOfflineRateSemiProduct.Rows.Count > 0)
            {
                string frequency = dtOfflineRateSemiProduct.Rows[0]["frequency"].ToString();
                dqmsBean.KpiId = id;
                dqmsBean.BuCode = dtOfflineRateSemiProduct.Rows[0]["bu_code"].ToString();
                dqmsBean.PlantCode = plant;
                DataTable dtSource = dqmsSqlOperater.GetOfflineRate(plant, "SFG");
                ...
                if (dtSource != null && dtSource.Rows.Count > 0)
                {
                    ...
                    double weight = double.Parse(dtOfflineRateSemiProduct.Rows[0]["weight"].ToString());
                    double lowerlimit = double.Parse(dtOfflineRateSemiProduct.Rows[0]["lower_limit"].ToString());
                    iniTrend = int.Parse(dtOfflineRateSemiProduct.Rows[0]["trend"].ToString());
                    dqmsBean.Score = OfflineRateSemiProduct.ComputationScore(dqmsBean.Actual, weight, lowerlimit);
                    ...
                    if (dtTmp != null && dtTmp.Rows.Count > 0)
                    {
                        ...
                        if (oldWorkDate != dqmsBean.WorkDate)
                        {
                            ...
                            dqmsBean.Trend = OfflineRateSemiProduct.CheckTrend(dqmsBean.Actual, dqmsBean.LastActual, iniTrend);

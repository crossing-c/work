...
namespace sDEM2100.Beans
{
    class OfflineRateProduct
    {
        ...
        public OfflineRateProduct(object[] clientInfo, string dbName, string plant)
        {
        ...
        public ExecutionResult DoOpreater(int id)
        {
            ...
            DataTable dtOfflineRateProduct = dqmsSqlOperater.GetDqmsParams(id, plant);
            ExecutionResult exeRes = new ExecutionResult();
            exeRes.Status = true;
            exeRes.Message = string.Empty;
            int iniTrend = 0;
            if (dtOfflineRateProduct != null && dtOfflineRateProduct.Rows.Count > 0)
            {
                string frequency = dtOfflineRateProduct.Rows[0]["frequency"].ToString();
                dqmsBean.KpiId = id;
                dqmsBean.BuCode = dtOfflineRateProduct.Rows[0]["bu_code"].ToString();
                dqmsBean.PlantCode = plant;
                DataTable dtSource = dqmsSqlOperater.GetOfflineRate(plant, "FG");
                ...
                if (dtSource != null && dtSource.Rows.Count > 0)
                {
                    ...
                    double weight = double.Parse(dtOfflineRateProduct.Rows[0]["weight"].ToString());
                    double lowerlimit = double.Parse(dtOfflineRateProduct.Rows[0]["lower_limit"].ToString());
                    iniTrend = int.Parse(dtOfflineRateProduct.Rows[0]["trend"].ToString());
                    dqmsBean.Score = OfflineRateProduct.ComputationScore(dqmsBean.Actual, weight,lowerlimit);
                    ...
                    if (dtTmp != null && dtTmp.Rows.Count > 0)
                    {
                        ...
                        if (oldWorkDate != dqmsBean.WorkDate)
                        {
                            ...
                            dqmsBean.Trend = OfflineRateProduct.CheckTrend(dqmsBean.Actual, dqmsBean.LastActual, iniTrend);

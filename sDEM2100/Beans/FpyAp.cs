...
namespace sDEM2100.Beans
{
    class FpyAp
    {
        ...
        public FpyAp(object[] clientInfo, string dbName,string plant)
        {
        ...
        public ExecutionResult DoOpreater(int id)
        {
            ...
            DataTable dtFpyAp = dqmsSqlOperater.GetDqmsParams(id, plant);
            ExecutionResult exeRes = new ExecutionResult();
            exeRes.Status = true;
            exeRes.Message = string.Empty;
            int iniTrend = 0;
            if (dtFpyAp != null && dtFpyAp.Rows.Count > 0)
            {
                string frequency = dtFpyAp.Rows[0]["frequency"].ToString();
                dqmsBean.KpiId = id;
                dqmsBean.BuCode = dtFpyAp.Rows[0]["bu_code"].ToString();
                dqmsBean.PlantCode = plant;
                DataTable dtSource = dqmsSqlOperater.GetFpyApSmt(plant, "AP");
                ...
                if (dtSource != null && dtSource.Rows.Count > 0)
                {
                    ...
                    double weight = double.Parse(dtFpyAp.Rows[0]["weight"].ToString());
                    double lowerlimit = double.Parse(dtFpyAp.Rows[0]["lower_limit"].ToString());
                    iniTrend = int.Parse(dtFpyAp.Rows[0]["trend"].ToString());
                    dqmsBean.Score = FpyAp.ComputationScore(dqmsBean.Actual, weight);
                    ...
                    if (dtTmp != null && dtTmp.Rows.Count > 0)
                    {
                        ...
                        if (oldWorkDate != dqmsBean.WorkDate)
                        {
                            ...
                            dqmsBean.Trend = FpyAp.CheckTrend(dqmsBean.Actual, dqmsBean.LastActual, iniTrend);

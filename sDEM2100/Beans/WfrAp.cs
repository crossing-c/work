...
namespace sDEM2100.Beans
{
    class WfrAp
    {
        ...
        public WfrAp(object[] clientInfo, string dbName, string plant)
        {
        ...
        public ExecutionResult DoOpreater(double actual, int id)
        {
            ...
            DataTable dtWfrAp = dqmsSqlOperater.GetDqmsParams(id, plant);
            ExecutionResult exeRes = new ExecutionResult();
            exeRes.Status = true;
            exeRes.Message = string.Empty;
            int iniTrend = 0;
            if (dtWfrAp != null && dtWfrAp.Rows.Count > 0)
            {
                string frequency = dtWfrAp.Rows[0]["frequency"].ToString();
                dqmsBean.KpiId = id;
                dqmsBean.BuCode = dtWfrAp.Rows[0]["bu_code"].ToString();
                dqmsBean.PlantCode = plant;
                dqmsBean.WorkDate = WorkDateConvert.GetWorkDate(frequency, 1);
                dqmsBean.Actual = actual;
                double weight = double.Parse(dtWfrAp.Rows[0]["weight"].ToString());
                double lowerlimit = double.Parse(dtWfrAp.Rows[0]["lower_limit"].ToString());
                iniTrend = int.Parse(dtWfrAp.Rows[0]["trend"].ToString());
                dqmsBean.Score = WfrAp.ComputationScore(dqmsBean.Actual, weight);
                ...
                if (flag)
                {
                    ...
                    //{
                    //再查一次 上周
                    double.TryParse(dqmsSqlOperater.GetPreWfrFatpSmt(plant, "FATP", WorkDateConvert.GetWorkDate(frequency, 0)), out lastActual);
                    lastScore = WfrAp.ComputationScore(lastActual, weight);
                    ...
                    //}
                    dqmsBean.Trend = WfrAp.CheckTrend(dqmsBean.Actual, dqmsBean.LastActual, iniTrend);

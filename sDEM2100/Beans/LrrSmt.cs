...
namespace sDEM2100.Beans
{
    class LrrSmt
    {
        ...
        public LrrSmt(object[] clientInfo, string dbName, string plant)
        {
        ...
        public ExecutionResult DoOpreater(int id)
        {
            ...
            DataTable dtLrrSmt = dqmsSqlOperater.GetDqmsParams(id, plant);
            ExecutionResult exeRes = new ExecutionResult();
            exeRes.Status = true;
            exeRes.Message = string.Empty;
            int iniTrend = 0;
            if (dtLrrSmt != null && dtLrrSmt.Rows.Count > 0)
            {
                string frequency = dtLrrSmt.Rows[0]["frequency"].ToString();
                dqmsBean.KpiId = id;
                dqmsBean.BuCode = dtLrrSmt.Rows[0]["bu_code"].ToString();
                dqmsBean.PlantCode = plant;
                DataTable dtSource = dqmsSqlOperater.GetLrrApSmt(plant, "SMT");
                ...
                if (dtSource != null && dtSource.Rows.Count > 0)
                {
                    ...
                    double weight = double.Parse(dtLrrSmt.Rows[0]["weight"].ToString());
                    //double lowerlimit = double.Parse(dtLrrSmt.Rows[0]["lower_limit"].ToString());
                    iniTrend = int.Parse(dtLrrSmt.Rows[0]["trend"].ToString());
                    dqmsBean.Score = LrrSmt.ComputationScore(dqmsBean.Actual, weight);
                    ...
                    if (dtTmp != null && dtTmp.Rows.Count > 0)
                    {
                        ...
                        if (oldWorkDate != dqmsBean.WorkDate)
                        {
                            ...
                            dqmsBean.Trend = LrrSmt.CheckTrend(dqmsBean.Actual, dqmsBean.LastActual, iniTrend);
